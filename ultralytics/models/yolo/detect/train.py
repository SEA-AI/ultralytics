...
class DetectionTrainer(BaseTrainer):
    ...
    def label_loss_items(self, loss_items=None, prefix: str = "train"):
        ...
        When ``single_cls_val`` is enabled, zero the validation classification loss so it does not
        dominate metrics while validating multi-class data as single-class.
        """
        if loss_items is None:
            return [f"{prefix}/{x}" for x in self.loss_names]
        items = {f"{prefix}/{k}": round(float(v), 5) for k, v in loss_items.items()}
        if prefix == "val" and getattr(self.args, "single_cls_val", False):
            cls_key = f"{prefix}/cls_loss"
            if cls_key in items:
                items[cls_key] = 0.0
        return items
