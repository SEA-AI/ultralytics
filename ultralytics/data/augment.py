...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz: int = 640, p: float = 1.0, n: int = 4, fill_value: int = 114):
        ...
        Args:
            ...
            p (float): Probability of applying the mosaic augmentation. Must be in the range 0-1.
            n (int): The grid size, either 4 (for 2x2) or 9 (for 3x3).
            fill_value (int): The fill value for the mosaic image.
        """
    ...
    def apply_image(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.n == 4:
            img4 = np.full((self.imgsz * 2, self.imgsz * 2, labels["img"].shape[2]), self.fill_value, dtype=np.uint8)
            for item in layout:
            ...
            img9 = np.full((self.imgsz * 3, self.imgsz * 3, labels["img"].shape[2]), self.fill_value, dtype=np.uint8)
            for item in layout:
                labels_patch = item["labels_patch"]
                img = labels_patch["img"]
...
class RandomPerspective(BaseTransform):
    ...
    def apply_image(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if (size[0] != img.shape[1] or size[1] != img.shape[0]) or (M != np.eye(3)).any():  # image changed
            ...
            fv = self.fill_value if isinstance(self.fill_value, int) else int(self.fill_value[0])
            fill_value = (fv, fv, fv, fv)
            if self.perspective:
                img = cv2.warpPerspective(img, M, dsize=size, borderValue=fill_value)
            else:  # affine
                img = cv2.warpAffine(img, M[:2], dsize=size, borderValue=fill_value)
            if img.ndim == 2:
                img = img[..., None]
        labels["img"] = img
...
class ClassifyLetterBox:
    ...
    def __init__(
        self, size: int | tuple[int, int] = (640, 640), auto: bool = False, stride: int = 32, fill_value: int = 114
    ):
